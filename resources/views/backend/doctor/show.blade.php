    ...
    <div class="container-fluid">
        <div class="page-titles">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="javascript:void(0)">Doctor Detail</a></li>
            </ol>
        </div>
        <div class="row">
            @if($doctor->patients)
                        ...
                        <div id="DZ_W_Todo2" class="widget-media dz-scroll ps ps--active-y height370">
                            <ul class="timeline">
                                ...
                                <li>
                                    <div class="timeline-panel bgl-dark flex-wrap border-0 p-3 rounded">
                                        <div class="media bg-transparent mr-2">
                                            <img class="rounded-circle" alt="image" width="48" src="{{ (($patient->image != '') && file_exists(public_path('images/patients/'.$patient->image))) ? asset('images/patients/'.$patient->image) : asset('backend/images/logo.png') }}">
                                        </div>
                                        <div class="media-body">
                                            <h5 class="mb-1 fs-18">{{$patient->name}}</h5>
                                            <span>{{$patient->treatment}}</span>
                                        </div>
                                        <ul class="mt-3 d-flex flex-wrap text-primary font-w600">
                                            <li class="mr-2">{{\Carbon\Carbon::parse($patient->check_in_date)->format('l F Y')}}</li>
                                            <li class="mr-2">{{\Carbon\Carbon::parse($patient->check_in_date)->format('H:i')}}- {{\Carbon\Carbon::parse($patient->check_in_date)->addMinutes(30)->format('h:i A')}}</li>
                                        </ul>
                                    </div>
                                </li>
                                @endforeach
                                ...
                                <li>
                                    <div class="timeline-panel bgl-dark flex-wrap border-0 p-3 rounded">
                                    <div class="media-body">
                                        <h5 class="mb-1 fs-18">No Shedule</h5>
                                        {{-- <span>{{$patient->treatment}}</span> --}}
                                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="media d-sm-flex d-block text-center text-sm-left pb-4 mb-4 border-bottom">
                            <img alt="image" class="rounded mr-sm-4 mr-0" width="130" src="{{ (($doctor->image != '') && file_exists(public_path('images/doctors/'.$doctor->image))) ? asset('images/doctors/'.$doctor->image) : asset('images/fallback-logo.png') }}">
                            <div class="media-body align-items-center">
                                <div class="d-sm-flex d-block justify-content-between my-3 my-sm-0">
                                    <div>
                                        <h3 class="fs-22 text-black font-w600 mb-0">{{$doctor->name}} </h3>
                            ...
                            <div class="col-lg-12 mb-lg-0 mb-3">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        ...
                                        <p class="fs-18 text-dark font-w600 mb-0">+977 {{$doctor->contact}}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-12 mb-3 mt-5">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        ...
                                        <p class="fs-18 text-dark font-w600 mb-0">{{$doctor->email}}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-xl-12 col-xxl-12">
                <div class="card">
                    <div class="card-header border-0 pb-0">
                        <h4 class="fs-20 font-w600">User Ratings and Reviews</h4>
                    </div>
                    <div class="card-body">
                        <!-- user ratings and reviews form -->
                        <form action="{{route('review.store')}}" method="POST" enctype="multipart/form-data" autocomplete="off">
                            @csrf
                        ...
                        <!-- user ratings and reviews list -->
                        @if($doctor->reviews->count() > 0)
                        <div class="mt-3">
                            <h5 class="mb-3">User Reviews:</h5>
                            @foreach($doctor->reviews as $review)
                            <div class="media mb-3">
                                <img alt="{{$review->user->name}}" class="rounded mr-3" width="60" src="{{ (($review->user->image != '') && file_exists(public_path('images/users/'.$review->user->image))) ?
                            asset('images/users/'.$review->user->image) : asset('backend/images/logo.png') }}">
                                <div class="media-body">
